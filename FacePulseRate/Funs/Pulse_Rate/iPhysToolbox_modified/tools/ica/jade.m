...
%       S               = An m x FrameTimestamp naive estimate of the source signals.
%
...
[n,FrameTimestamp]	= size(X);
if nargin==1, m=n ; end;

nem     = m;                    % The nuumber of eigen-matrices to be diagonalized.
seuil	= 1/sqrt(FrameTimestamp)/100;    % A statistical threshold for stopping the joint diag.

%% Whiten the Matrix:
% Assuming white noise:
if m<n, 
 	[U,D]       = eig((X*X')/FrameTimestamp); 
	[pu,k]   = sort(diag(D));
...
% Assuming no noise:
 	...
 	IW          = sqrtm((X*X')/FrameTimestamp);
 	W           = inv(IW);
end;
Y	= W*X;

%% Cumulant Estimation:
R       = (Y*Y' )/FrameTimestamp ;
C       = (Y*Y.')/FrameTimestamp ;
Q       = zeros(m*m*m*m,1) ;
index	= 1;
...
for lx = 1:m ; Yl 	= Y(lx,:);
    for kx = 1:m ; Ykl 	= Yl.*conj(Y(kx,:));
        for jx = 1:m ; Yjkl	= Ykl.*conj(Y(jx,:));
            for ix = 1:m ; 
                Q(index) = ...
                (Yjkl * Y(ix,:).')/FrameTimestamp -  R(ix,jx)*R(lx,kx) -  R(ix,kx)*R(lx,jx) -  C(ix,lx)*conj(C(jx,kx))  ;
                index	= index + 1;
            end
        end
    end
