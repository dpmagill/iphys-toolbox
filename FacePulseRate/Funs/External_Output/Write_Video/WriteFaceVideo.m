...
%   skin segmentation, and pulse rate. When flag OutputConfig.WriteVideoDetailedDiagnosticsTF is  
%   true, additional detailed annotations are displayed; these annotations are intended only for 
%   internal development. This flag can be toggled to true within function 
%   ValidateAndConfigure_InternalFlags. Examples of the annotations are shown below. 
%
...
%   
%   -- Use of Default Display Option Annotations --
%
%       There are generally two categories of adjustments that can be made. One category affects 
%       the placement of ROIs and one category affects skin segmentation.
%      
%           - Incorrect placement: 
%
%             Six courses of action can be taken regarding incorrect placement.              
%
%             (1): Without modifying any settings, a misplaced ROI can be ignored by argument 
%                  ROIIgnoreByArgument or replaced by argument ROISpecifyByArgument. After the 
%                  video has been reprocessed with one or both of these arguments used, the use of    
%                  the arguments can be verified by inspecting the output video: frames that were   
%                  modified by the arguments will contain indications of their use (displayed above 
%                  the ROI).
%
%             (2): To adjust the size of the ROI, adjust arguments ROIWidthResizeFactor and/or 
%                  ROIHeightResizeFactor. Increasing these values increases the ROI width and 
%                  height respectively.
%
%             (3): If the half-rectangles in the lower right-hand side are noticably larger than 
...
%                  detectors (see function FaceDetect). This can be remedied by decreasing the 
%                  value of arguments ROIWidthResizeFactor and/or ROIHeightResizeFactor.
%
%             (4): The profile (secondary #1) face-detection algorithm tends to be less accurate 
%                  than the frontal (primary) face-detection algorithm. If the profile algorithm  
%                  appears to be responsible for many misplacements, it can be disabled. The 
%                  algorithm that determined the ROI is listed on the top of the ROI box. Disable
%                  the profile detector by specifying argument ProfileFaceDetectorTF as false.
%
%             (5): An ROI verification algorithm (for face detection, see function 
%                  FaceDetect_ROIValidate; for skin detection, see function SkinDetect_ROIValidate)  
%                  accepts or rejects ROIs returned by detection algorithms. The first step of
%                  verification involves applying a severe range of skin segmentation to the ROI.
%                  Note that this range is different from the range used for skin segmentation
%                  applied when taking the RGB means; that is, it is different from the range
%                  specified by SkinSegmentThresholdsGenericYCbCr and 
%                  SkinSegmentThresholdsGenericHSV. However, as the range used for verification is
%                  constrained by SkinSegmentThresholdsGenericYCbCr and 
%                  SkinSegmentThresholdsGenericHSV, severe settings for 
%                  SkinSegmentThresholdsGenericYCbCr and SkinSegmentThresholdsGenericHSV could 
%                  cause the range used for verification to be too severe. If the severity of the 
%                  range is too high, this could lead ROIs being rejected too frequently. When an 
%                  ROI is rejected, the annotation "R" will appear. If this annotation appears too
%                  frequently, this may indicate the severity is too great. For ROIs returned by 
%                  the skin-detection algorithm, the annotation "NR" may also be observed 
%                  frequently if the skin segmentation range used for verification is too severe 
%                  ("NR" means no skin regions were available; see function 
%                  SkinDetect_PartitionRegions"). To reduce the severity of the skin segmentation
%                  range used for verification, increase the range specified by arguments 
%                  DetectVerifyPercentilesYCbCrH and DetectVerifyPercentilesS.
%
%             (6): Conversely, the ROI verification algorithm may be too lenient, resulting in ROIs
%                  not being rejected when they should be. This is likely due to there being an
%                  insufficient number of skin-color samples from the video for verification
%                  mechanisms that require a minimum number of samples (these mechamisms include 
%                  tailored skin segmentation and skin-detection used in the service of 
%                  ROI verification; see function ConfirmSkinPresent_SkinDetectReject). These 
%                  mechanisms will not be activated if the required number of skin-color samples is  
%                  not present. The legend will indicate whether tailored skin segmentation and/or  
%                  the skin-detection algorithm could not be activated due to a low number of   
%                  samples. Skin-color samples are collected only from ROIs returned by the frontal  
%                  (primary) face-detection algorithm, so a lack of detections by this algorithm is  
%                  likely the cause of too few samples. To increase the number of samples, consider  
%                  increasing the duration of the video that is assessed.
%
%           - Skin-segmentation too severe or lenient
%
%             Skin segmentation refers to excluding pixels that are classified as non-skin by the 
%             skin-segmentation algorithm. The purpose of skin segmentation is to make the face and 
%             skin measurments more robust to environmental changes across the video. Specifically, 
%             anything that changes in the environment could be interpreted as a change in skin 
%             coloration if not excluded. Environmental features that remain constant and are 
%             consistently included in each ROI are not of concern as these features will not 
%             produces changes in skin coloration across frames. Increasing the exclusion of 
%             environmental features within the ROI may have the side effect of increasing the 
%             exclusion of skin areas. Excluding skin areas may become detrimental  if it results 
%             in the location of the face from which measurements are taken changing across frames. 
%             It can also be detrimental if the number of pixels from which measurements are taken 
%             changes across frames. These two events will be detrimental if they lead to changes 
%             to measurements that are not due solely to changes in skin coloration due to blood 
%             oxygenation levels. Therefore, the extent of excluding environmental features should 
%             be balanced with the exclusion of skin areas.
%
%             (1): General severity factor: To increase or decrease segmentation severity without 
%                  the need to adjust thresholds, consider increasing or decreasing the severity 
%                  factor. Increasing the value of the factor results in more severe segmentation.
%                  Note that this factor cannot remove segmentation enforced by the permanent
%                  thresholds. However, segmentation is determined by several factors in addition  
%                  to the permanent thresholds, so differences in segmentation should generally be
%                  expected when adjusting this factor. Adjust the factor by argument 
%                  SkinSegmentSeverityFactor. A larger value results in greater segmentation.
%
%             (2): Permanent thresholds: The permanent thresholds are hard limitiations on which 
%                  pixel colors are considered skin. Consider changing these if over- or under- 
%                  segmentation may be due to environmental conditions. For example, if the face is 
%                  in a shadow, it may be necessary to allow pixels that are darker than the
%                  default permanent thresholds permit (i.e., reduce the Y channel minimum 
%                  treshold to its minimum, which is 16). Sometimes, videos may have a 
%                  characteristic tint that requires other permanent thresholds to be changed.
%                  Change the range of permanent thesholds by adjusting arguments
...
%                  indicates the maximum of the range. 
%
%             (C): Oversegmentation adjustments: By default, the presence of oversegmentation is
%                  checked. If oversegmentation is determined to be present, some skin-segmentation
%                  settings will be made less severe. Sometimes, oversegmentation will be detected 
%                  when it doesn't exist. For example, the presense of hair occluding part of the 
%                  face can sometimes lead the oversegmentation check to determine that too much of 
%                  the face was segmented; in this case, the cause of much segmentation of the face 
%                  is due to proper segmentation of hair rather than general oversegmentation.  
%                  Whether settings were made less severe will be indicated by text on the legend: 
%                  "Oversegmentation Adjustments: true". If skin-segmentation settings have been 
%                  made less severe, and the resulting skin segmentation appears too lenient, the 
