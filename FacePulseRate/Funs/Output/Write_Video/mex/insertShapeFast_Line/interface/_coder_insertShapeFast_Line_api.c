 ...
 *
 * _coder_insertShapeFast_Line_api.c
 *
 * Code generation for function '_coder_insertShapeFast_Line_api'
 *
 */
...
static int16_T (*d_emlrt_marshallIn(const mxArray *u, const emlrtMsgIdentifier
  *parentId))[4];
static real_T e_emlrt_marshallIn(const mxArray *LineLineWidth, const char_T
  *identifier);
  ...
  *identifier)
{
  ...
  thisId.fParent = NULL;
  thisId.bParentIsCell = false;
  y = f_emlrt_marshallIn(emlrtAlias(LineLineWidth), &thisId);
  emlrtDestroyArray(&LineLineWidth);
  return y;
}
...
void insertShapeFast_Line_api(const mxArray * const prhs[3], int32_T nlhs, const
  mxArray *plhs[1])
{
  emxArray_uint8_T *FrameCropped_Markers;
  const mxArray *prhs_copy_idx_0;
  int16_T (*MarkerPointsXY_Cropped)[4];
  real_T LineLineWidth;
  (void)nlhs;
  ...
  LineLineWidth = e_emlrt_marshallIn(emlrtAliasP(prhs[2]), "LineLineWidth");
...
/* End of code generation (_coder_insertShapeFast_Line_api.c) */
