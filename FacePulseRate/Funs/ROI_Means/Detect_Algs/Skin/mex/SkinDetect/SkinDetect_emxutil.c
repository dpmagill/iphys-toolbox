/*
 * Academic License - for use in teaching, academic research, and meeting
 * course requirements at degree granting institutions only.  Not for
 * government, commercial, or other organizational use.
 *
 * SkinDetect_emxutil.c
 *
 * Code generation for function 'SkinDetect_emxutil'
 *
 */

/* Include files */
#include "SkinDetect_emxutil.h"
#include "SkinDetect_data.h"
#include "SkinDetect_types.h"
#include "rt_nonfinite.h"
#include <string.h>

/* Function Definitions */
void emxCopyMatrix_real_T(real_T dst[2], const real_T src[2])
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    dst[i] = src[i];
  }
}

void emxCopyMatrix_real_T1(real_T dst[4], const real_T src[4])
{
  int32_T i;
  for (i = 0; i < 4; i++) {
    dst[i] = src[i];
  }
}

void emxCopyMatrix_real_T2(real_T dst[16], const real_T src[16])
{
  int32_T i;
  for (i = 0; i < 16; i++) {
    dst[i] = src[i];
  }
}

void emxCopyStruct_struct_T(c_struct_T *dst, const c_struct_T *src)
{
  dst->Area = src->Area;
  emxCopyMatrix_real_T(dst->Centroid, src->Centroid);
  emxCopyMatrix_real_T1(dst->BoundingBox, src->BoundingBox);
  dst->MajorAxisLength = src->MajorAxisLength;
  dst->MinorAxisLength = src->MinorAxisLength;
  dst->Eccentricity = src->Eccentricity;
  dst->Orientation = src->Orientation;
  emxCopy_boolean_T_0x0(&dst->Image, &src->Image);
  emxCopy_boolean_T_0x0(&dst->FilledImage, &src->FilledImage);
  dst->FilledArea = src->FilledArea;
  dst->EulerNumber = src->EulerNumber;
  emxCopyMatrix_real_T2(dst->Extrema, src->Extrema);
  dst->EquivDiameter = src->EquivDiameter;
  dst->Extent = src->Extent;
  emxCopy_real_T(&dst->PixelIdxList, &src->PixelIdxList);
  emxCopy_real_T(&dst->PixelList, &src->PixelList);
  dst->Perimeter = src->Perimeter;
  dst->Circularity = src->Circularity;
  emxCopy_real_T_0(&dst->PixelValues, &src->PixelValues);
  emxCopyMatrix_real_T(dst->WeightedCentroid, src->WeightedCentroid);
  dst->MeanIntensity = src->MeanIntensity;
  dst->MinIntensity = src->MinIntensity;
  dst->MaxIntensity = src->MaxIntensity;
  emxCopy_real_T_1x0(&dst->SubarrayIdx, &src->SubarrayIdx);
  emxCopyMatrix_real_T(dst->SubarrayIdxLengths, src->SubarrayIdxLengths);
}

void emxCopy_boolean_T_0x0(emxArray_boolean_T_0x0 *dst, const
  emxArray_boolean_T_0x0 *src)
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    dst->size[i] = src->size[i];
  }
}

void emxCopy_real_T(emxArray_real_T **dst, emxArray_real_T * const *src)
{
  int32_T i;
  int32_T numElDst;
  int32_T numElSrc;
  numElDst = 1;
  numElSrc = 1;
  for (i = 0; i < (*dst)->numDimensions; i++) {
    numElDst *= (*dst)->size[i];
    numElSrc *= (*src)->size[i];
  }

  for (i = 0; i < (*dst)->numDimensions; i++) {
    (*dst)->size[i] = (*src)->size[i];
  }

  emxEnsureCapacity_real_T(*dst, numElDst);
  for (i = 0; i < numElSrc; i++) {
    (*dst)->data[i] = (*src)->data[i];
  }
}

void emxCopy_real_T_0(emxArray_real_T_0 *dst, const emxArray_real_T_0 *src)
{
  dst->size[0] = src->size[0];
}

void emxCopy_real_T_1x0(emxArray_real_T_1x0 *dst, const emxArray_real_T_1x0 *src)
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    dst->size[i] = src->size[i];
  }
}

void emxEnsureCapacity_boolean_T(emxArray_boolean_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(boolean_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(boolean_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (boolean_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }
}

void emxEnsureCapacity_int16_T(emxArray_int16_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(int16_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(int16_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (int16_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }
}

void emxEnsureCapacity_int32_T(emxArray_int32_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(int32_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(int32_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (int32_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }
}

void emxEnsureCapacity_real32_T(emxArray_real32_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(real32_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(real32_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (real32_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }
}

void emxEnsureCapacity_real_T(emxArray_real_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(real_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(real_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (real_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }
}

void emxEnsureCapacity_struct_T(b_struct_T data[400], const int32_T size[1],
  int32_T oldNumel)
{
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  if (oldNumel > size[0]) {
    emxTrim_struct_T_400(data, size[0], oldNumel);
  } else {
    if (oldNumel < size[0]) {
      emxExpand_struct_T_400(data, oldNumel, size[0]);
    }
  }
}

void emxEnsureCapacity_struct_T1(emxArray_struct_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(struct_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(struct_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (struct_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }
}

void emxEnsureCapacity_struct_T2(b_emxArray_struct_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(c_struct_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(c_struct_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (c_struct_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }

  if (oldNumel > newNumel) {
    emxTrim_struct_T(emxArray, newNumel, oldNumel);
  } else {
    if (oldNumel < newNumel) {
      emxExpand_struct_T(emxArray, oldNumel, newNumel);
    }
  }
}

void emxEnsureCapacity_uint32_T(emxArray_uint32_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(uint32_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(uint32_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (uint32_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }
}

void emxEnsureCapacity_uint8_T(emxArray_uint8_T *emxArray, int32_T oldNumel)
{
  int32_T i;
  int32_T newNumel;
  void *newData;
  if (oldNumel < 0) {
    oldNumel = 0;
  }

  newNumel = 1;
  for (i = 0; i < emxArray->numDimensions; i++) {
    newNumel *= emxArray->size[i];
  }

  if (newNumel > emxArray->allocatedSize) {
    i = emxArray->allocatedSize;
    if (i < 16) {
      i = 16;
    }

    while (i < newNumel) {
      if (i > 1073741823) {
        i = MAX_int32_T;
      } else {
        i *= 2;
      }
    }

    newData = emlrtCallocMex((uint32_T)i, sizeof(uint8_T));
    if (emxArray->data != NULL) {
      memcpy(newData, emxArray->data, sizeof(uint8_T) * oldNumel);
      if (emxArray->canFreeData) {
        emlrtFreeMex(emxArray->data);
      }
    }

    emxArray->data = (uint8_T *)newData;
    emxArray->allocatedSize = i;
    emxArray->canFreeData = true;
  }
}

void emxExpand_struct_T(b_emxArray_struct_T *emxArray, int32_T fromIndex,
  int32_T toIndex)
{
  int32_T i;
  for (i = fromIndex; i < toIndex; i++) {
    emxInitStruct_struct_T2(&emxArray->data[i], false);
  }
}

void emxExpand_struct_T_400(b_struct_T data[400], int32_T fromIndex, int32_T
  toIndex)
{
  int32_T i;
  for (i = fromIndex; i < toIndex; i++) {
    emxInitStruct_struct_T(&data[i], false);
  }
}

void emxFreeMatrix_cell_wrap_1(cell_wrap_1 pMatrix[5])
{
  int32_T i;
  for (i = 0; i < 5; i++) {
    emxFreeStruct_cell_wrap_1(&pMatrix[i]);
  }
}

void emxFreeMatrix_cell_wrap_2(cell_wrap_2 pMatrix[2])
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    emxFreeStruct_cell_wrap_2(&pMatrix[i]);
  }
}

void emxFreeMatrix_cell_wrap_3(cell_wrap_3 pMatrix[2])
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    emxFreeStruct_cell_wrap_3(&pMatrix[i]);
  }
}

void emxFreeMatrix_cell_wrap_32(cell_wrap_32 pMatrix[3])
{
  int32_T i;
  for (i = 0; i < 3; i++) {
    emxFreeStruct_cell_wrap_32(&pMatrix[i]);
  }
}

void emxFreeStruct_cell_wrap_1(cell_wrap_1 *pStruct)
{
  emxFree_real32_T(&pStruct->f1);
}

void emxFreeStruct_cell_wrap_2(cell_wrap_2 *pStruct)
{
  emxFreeMatrix_cell_wrap_1(pStruct->f1);
}

void emxFreeStruct_cell_wrap_3(cell_wrap_3 *pStruct)
{
  emxFree_boolean_T(&pStruct->f1);
}

void emxFreeStruct_cell_wrap_32(cell_wrap_32 *pStruct)
{
  emxFree_uint32_T(&pStruct->f1);
}

void emxFreeStruct_struct5_T(struct5_T *pStruct)
{
  emxFree_real_T(&pStruct->ROISpecifyByArgument);
  emxFree_real_T(&pStruct->ROIIgnoreByArgument);
}

void emxFreeStruct_struct8_T(struct8_T *pStruct)
{
  emxFree_uint32_T(&pStruct->RegionBoundaries_SelectedLinIdx);
  emxFree_uint32_T(&pStruct->c_RegionBoundaries_CandidateLin);
  emxFree_uint32_T(&pStruct->RegionBoundaries_RejectedLinIdx);
}

void emxFreeStruct_struct_T(b_struct_T *pStruct)
{
  emxFree_uint32_T(&pStruct->PixelIdxList);
}

void emxFreeStruct_struct_T1(d_struct_T *pStruct)
{
  emxFree_uint32_T(&pStruct->RegionBoundaries_SelectedLinIdx);
  emxFree_uint32_T(&pStruct->c_RegionBoundaries_CandidateLin);
  emxFree_uint32_T(&pStruct->RegionBoundaries_RejectedLinIdx);
}

void emxFreeStruct_struct_T2(c_struct_T *pStruct)
{
  emxFree_real_T(&pStruct->PixelIdxList);
  emxFree_real_T(&pStruct->PixelList);
}

void emxFree_boolean_T(emxArray_boolean_T **pEmxArray)
{
  if (*pEmxArray != (emxArray_boolean_T *)NULL) {
    if (((*pEmxArray)->data != (boolean_T *)NULL) && (*pEmxArray)->canFreeData)
    {
      emlrtFreeMex((*pEmxArray)->data);
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (emxArray_boolean_T *)NULL;
  }
}

void emxFree_int16_T(emxArray_int16_T **pEmxArray)
{
  if (*pEmxArray != (emxArray_int16_T *)NULL) {
    if (((*pEmxArray)->data != (int16_T *)NULL) && (*pEmxArray)->canFreeData) {
      emlrtFreeMex((*pEmxArray)->data);
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (emxArray_int16_T *)NULL;
  }
}

void emxFree_int32_T(emxArray_int32_T **pEmxArray)
{
  if (*pEmxArray != (emxArray_int32_T *)NULL) {
    if (((*pEmxArray)->data != (int32_T *)NULL) && (*pEmxArray)->canFreeData) {
      emlrtFreeMex((*pEmxArray)->data);
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (emxArray_int32_T *)NULL;
  }
}

void emxFree_real32_T(emxArray_real32_T **pEmxArray)
{
  if (*pEmxArray != (emxArray_real32_T *)NULL) {
    if (((*pEmxArray)->data != (real32_T *)NULL) && (*pEmxArray)->canFreeData) {
      emlrtFreeMex((*pEmxArray)->data);
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (emxArray_real32_T *)NULL;
  }
}

void emxFree_real_T(emxArray_real_T **pEmxArray)
{
  if (*pEmxArray != (emxArray_real_T *)NULL) {
    if (((*pEmxArray)->data != (real_T *)NULL) && (*pEmxArray)->canFreeData) {
      emlrtFreeMex((*pEmxArray)->data);
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (emxArray_real_T *)NULL;
  }
}

void emxFree_struct_T(emxArray_struct_T **pEmxArray)
{
  if (*pEmxArray != (emxArray_struct_T *)NULL) {
    if (((*pEmxArray)->data != (struct_T *)NULL) && (*pEmxArray)->canFreeData) {
      emlrtFreeMex((*pEmxArray)->data);
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (emxArray_struct_T *)NULL;
  }
}

void emxFree_struct_T1(b_emxArray_struct_T **pEmxArray)
{
  int32_T i;
  int32_T numEl;
  if (*pEmxArray != (b_emxArray_struct_T *)NULL) {
    if ((*pEmxArray)->data != (c_struct_T *)NULL) {
      numEl = 1;
      for (i = 0; i < (*pEmxArray)->numDimensions; i++) {
        numEl *= (*pEmxArray)->size[i];
      }

      for (i = 0; i < numEl; i++) {
        emxFreeStruct_struct_T2(&(*pEmxArray)->data[i]);
      }

      if ((*pEmxArray)->canFreeData) {
        emlrtFreeMex((*pEmxArray)->data);
      }
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (b_emxArray_struct_T *)NULL;
  }
}

void emxFree_struct_T_400(emxArray_struct_T_400 *pEmxArray)
{
  int32_T i;
  int32_T numEl;
  numEl = 1;
  for (i = 0; i < 1; i++) {
    numEl *= pEmxArray->size[0];
  }

  for (i = 0; i < numEl; i++) {
    emxFreeStruct_struct_T(&pEmxArray->data[i]);
  }
}

void emxFree_uint32_T(emxArray_uint32_T **pEmxArray)
{
  if (*pEmxArray != (emxArray_uint32_T *)NULL) {
    if (((*pEmxArray)->data != (uint32_T *)NULL) && (*pEmxArray)->canFreeData) {
      emlrtFreeMex((*pEmxArray)->data);
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (emxArray_uint32_T *)NULL;
  }
}

void emxFree_uint8_T(emxArray_uint8_T **pEmxArray)
{
  if (*pEmxArray != (emxArray_uint8_T *)NULL) {
    if (((*pEmxArray)->data != (uint8_T *)NULL) && (*pEmxArray)->canFreeData) {
      emlrtFreeMex((*pEmxArray)->data);
    }

    emlrtFreeMex((*pEmxArray)->size);
    emlrtFreeMex(*pEmxArray);
    *pEmxArray = (emxArray_uint8_T *)NULL;
  }
}

void emxInitMatrix_cell_wrap_1(cell_wrap_1 pMatrix[5], boolean_T doPush)
{
  int32_T i;
  for (i = 0; i < 5; i++) {
    emxInitStruct_cell_wrap_1(&pMatrix[i], doPush);
  }
}

void emxInitMatrix_cell_wrap_2(cell_wrap_2 pMatrix[2], boolean_T doPush)
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    emxInitStruct_cell_wrap_2(&pMatrix[i], doPush);
  }
}

void emxInitMatrix_cell_wrap_3(cell_wrap_3 pMatrix[2], boolean_T doPush)
{
  int32_T i;
  for (i = 0; i < 2; i++) {
    emxInitStruct_cell_wrap_3(&pMatrix[i], doPush);
  }
}

void emxInitMatrix_cell_wrap_32(cell_wrap_32 pMatrix[3], boolean_T doPush)
{
  int32_T i;
  for (i = 0; i < 3; i++) {
    emxInitStruct_cell_wrap_32(&pMatrix[i], doPush);
  }
}

void emxInitStruct_cell_wrap_1(cell_wrap_1 *pStruct, boolean_T doPush)
{
  emxInit_real32_T(&pStruct->f1, 2, doPush);
}

void emxInitStruct_cell_wrap_2(cell_wrap_2 *pStruct, boolean_T doPush)
{
  emxInitMatrix_cell_wrap_1(pStruct->f1, doPush);
}

void emxInitStruct_cell_wrap_3(cell_wrap_3 *pStruct, boolean_T doPush)
{
  emxInit_boolean_T(emlrtRootTLSGlobal, &pStruct->f1, 2, doPush);
}

void emxInitStruct_cell_wrap_32(cell_wrap_32 *pStruct, boolean_T doPush)
{
  emxInit_uint32_T(&pStruct->f1, 1, doPush);
}

void emxInitStruct_struct5_T(struct5_T *pStruct, boolean_T doPush)
{
  emxInit_real_T(&pStruct->ROISpecifyByArgument, 2, doPush);
  emxInit_real_T(&pStruct->ROIIgnoreByArgument, 1, doPush);
}

void emxInitStruct_struct8_T(struct8_T *pStruct, boolean_T doPush)
{
  pStruct->BBox_Scan_SkinAlg.size[0] = 0;
  pStruct->BBox_Scan_SkinAlg.size[1] = 0;
  pStruct->RegionNPixels_RejectedN.size[0] = 0;
  pStruct->RegionNPixels_RejectedN.size[1] = 0;
  pStruct->RegionNPixels_RejectedLowNTF.size[0] = 0;
  pStruct->RegionNPixels_RejectedLowNTF.size[1] = 0;
  pStruct->RegionCentroids_SelectedXY.size[0] = 0;
  pStruct->RegionCentroids_SelectedXY.size[1] = 0;
  pStruct->RegionCentroids_CandidateXY.size[0] = 0;
  pStruct->RegionCentroids_CandidateXY.size[1] = 0;
  pStruct->RegionCentroids_RejectedXY.size[0] = 0;
  pStruct->RegionCentroids_RejectedXY.size[1] = 0;
  pStruct->PredictedROI_center.size[0] = 0;
  pStruct->PredictedROI_center.size[1] = 0;
  pStruct->RegionScores_SelectedRGBProb.size[0] = 0;
  pStruct->RegionScores_SelectedRGBProb.size[1] = 0;
  pStruct->RegionScores_CandidateRGBProb.size[0] = 0;
  pStruct->RegionScores_CandidateRGBProb.size[1] = 0;
  pStruct->RegionScores_RejectedRGBProb.size[0] = 0;
  pStruct->RegionScores_RejectedRGBProb.size[1] = 0;
  pStruct->RegionScores_SelectedYCbCrZ.size[0] = 0;
  pStruct->RegionScores_SelectedYCbCrZ.size[1] = 0;
  pStruct->RegionScores_CandidateYCbCrZ.size[0] = 0;
  pStruct->RegionScores_CandidateYCbCrZ.size[1] = 0;
  pStruct->RegionScores_RejectedYCbCrZ.size[0] = 0;
  pStruct->RegionScores_RejectedYCbCrZ.size[1] = 0;
  pStruct->RegionRankings_CandidateHi2Lo.size[0] = 0;
  pStruct->RegionRankings_CandidateHi2Lo.size[1] = 0;
  pStruct->c_RegionCrSDScores_RejectedCrSD.size[0] = 0;
  pStruct->c_RegionCrSDScores_RejectedCrSD.size[1] = 0;
  pStruct->RegionCrSDScores_RejectedCrSD.size[0] = 0;
  pStruct->RegionCrSDScores_RejectedCrSD.size[1] = 0;
  emxInit_uint32_T(&pStruct->RegionBoundaries_SelectedLinIdx, 2, doPush);
  emxInit_uint32_T(&pStruct->c_RegionBoundaries_CandidateLin, 2, doPush);
  emxInit_uint32_T(&pStruct->RegionBoundaries_RejectedLinIdx, 2, doPush);
}

void emxInitStruct_struct_T(b_struct_T *pStruct, boolean_T doPush)
{
  emxInit_uint32_T(&pStruct->PixelIdxList, 2, doPush);
}

void emxInitStruct_struct_T1(d_struct_T *pStruct, boolean_T doPush)
{
  pStruct->BBox_Scan_SkinAlg.size[0] = 0;
  pStruct->BBox_Scan_SkinAlg.size[1] = 0;
  pStruct->RegionNPixels_RejectedN.size[0] = 0;
  pStruct->RegionNPixels_RejectedN.size[1] = 0;
  pStruct->RegionNPixels_RejectedLowNTF.size[0] = 0;
  pStruct->RegionNPixels_RejectedLowNTF.size[1] = 0;
  pStruct->RegionCentroids_SelectedXY.size[0] = 0;
  pStruct->RegionCentroids_SelectedXY.size[1] = 0;
  pStruct->RegionCentroids_CandidateXY.size[0] = 0;
  pStruct->RegionCentroids_CandidateXY.size[1] = 0;
  pStruct->RegionCentroids_RejectedXY.size[0] = 0;
  pStruct->RegionCentroids_RejectedXY.size[1] = 0;
  pStruct->PredictedROI_center.size[0] = 0;
  pStruct->PredictedROI_center.size[1] = 0;
  pStruct->RegionScores_SelectedRGBProb.size[0] = 0;
  pStruct->RegionScores_SelectedRGBProb.size[1] = 0;
  pStruct->RegionScores_CandidateRGBProb.size[0] = 0;
  pStruct->RegionScores_CandidateRGBProb.size[1] = 0;
  pStruct->RegionScores_RejectedRGBProb.size[0] = 0;
  pStruct->RegionScores_RejectedRGBProb.size[1] = 0;
  pStruct->RegionScores_SelectedYCbCrZ.size[0] = 0;
  pStruct->RegionScores_SelectedYCbCrZ.size[1] = 0;
  pStruct->RegionScores_CandidateYCbCrZ.size[0] = 0;
  pStruct->RegionScores_CandidateYCbCrZ.size[1] = 0;
  pStruct->RegionScores_RejectedYCbCrZ.size[0] = 0;
  pStruct->RegionScores_RejectedYCbCrZ.size[1] = 0;
  pStruct->RegionRankings_CandidateHi2Lo.size[0] = 0;
  pStruct->RegionRankings_CandidateHi2Lo.size[1] = 0;
  pStruct->c_RegionCrSDScores_RejectedCrSD.size[0] = 0;
  pStruct->c_RegionCrSDScores_RejectedCrSD.size[1] = 0;
  pStruct->RegionCrSDScores_RejectedCrSD.size[0] = 0;
  pStruct->RegionCrSDScores_RejectedCrSD.size[1] = 0;
  emxInit_uint32_T(&pStruct->RegionBoundaries_SelectedLinIdx, 2, doPush);
  emxInit_uint32_T(&pStruct->c_RegionBoundaries_CandidateLin, 2, doPush);
  emxInit_uint32_T(&pStruct->RegionBoundaries_RejectedLinIdx, 2, doPush);
}

void emxInitStruct_struct_T2(c_struct_T *pStruct, boolean_T doPush)
{
  pStruct->Image.size[0] = 0;
  pStruct->Image.size[1] = 0;
  pStruct->FilledImage.size[0] = 0;
  pStruct->FilledImage.size[1] = 0;
  emxInit_real_T(&pStruct->PixelIdxList, 1, doPush);
  emxInit_real_T(&pStruct->PixelList, 2, doPush);
  pStruct->PixelValues.size[0] = 0;
  pStruct->SubarrayIdx.size[0] = 0;
  pStruct->SubarrayIdx.size[1] = 0;
}

void emxInit_boolean_T(emlrtCTX aTLS, emxArray_boolean_T **pEmxArray, int32_T
  numDimensions, boolean_T doPush)
{
  emxArray_boolean_T *emxArray;
  int32_T i;
  *pEmxArray = (emxArray_boolean_T *)emlrtMallocMex(sizeof(emxArray_boolean_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(aTLS, (void *)pEmxArray, (void *)
      &emxFree_boolean_T);
  }

  emxArray = *pEmxArray;
  emxArray->data = (boolean_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxInit_int16_T(emxArray_int16_T **pEmxArray, int32_T numDimensions,
                     boolean_T doPush)
{
  emxArray_int16_T *emxArray;
  int32_T i;
  *pEmxArray = (emxArray_int16_T *)emlrtMallocMex(sizeof(emxArray_int16_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(emlrtRootTLSGlobal, (void *)pEmxArray,
      (void *)&emxFree_int16_T);
  }

  emxArray = *pEmxArray;
  emxArray->data = (int16_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxInit_int32_T(emxArray_int32_T **pEmxArray, int32_T numDimensions,
                     boolean_T doPush)
{
  emxArray_int32_T *emxArray;
  int32_T i;
  *pEmxArray = (emxArray_int32_T *)emlrtMallocMex(sizeof(emxArray_int32_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(emlrtRootTLSGlobal, (void *)pEmxArray,
      (void *)&emxFree_int32_T);
  }

  emxArray = *pEmxArray;
  emxArray->data = (int32_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxInit_real32_T(emxArray_real32_T **pEmxArray, int32_T numDimensions,
                      boolean_T doPush)
{
  emxArray_real32_T *emxArray;
  int32_T i;
  *pEmxArray = (emxArray_real32_T *)emlrtMallocMex(sizeof(emxArray_real32_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(emlrtRootTLSGlobal, (void *)pEmxArray,
      (void *)&emxFree_real32_T);
  }

  emxArray = *pEmxArray;
  emxArray->data = (real32_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxInit_real_T(emxArray_real_T **pEmxArray, int32_T numDimensions,
                    boolean_T doPush)
{
  emxArray_real_T *emxArray;
  int32_T i;
  *pEmxArray = (emxArray_real_T *)emlrtMallocMex(sizeof(emxArray_real_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(emlrtRootTLSGlobal, (void *)pEmxArray,
      (void *)&emxFree_real_T);
  }

  emxArray = *pEmxArray;
  emxArray->data = (real_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxInit_struct_T(emxArray_struct_T **pEmxArray, int32_T numDimensions,
                      boolean_T doPush)
{
  emxArray_struct_T *emxArray;
  int32_T i;
  *pEmxArray = (emxArray_struct_T *)emlrtMallocMex(sizeof(emxArray_struct_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(emlrtRootTLSGlobal, (void *)pEmxArray,
      (void *)&emxFree_struct_T);
  }

  emxArray = *pEmxArray;
  emxArray->data = (struct_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxInit_struct_T1(b_emxArray_struct_T **pEmxArray, int32_T numDimensions,
  boolean_T doPush)
{
  b_emxArray_struct_T *emxArray;
  int32_T i;
  *pEmxArray = (b_emxArray_struct_T *)emlrtMallocMex(sizeof(b_emxArray_struct_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(emlrtRootTLSGlobal, (void *)pEmxArray,
      (void *)&emxFree_struct_T1);
  }

  emxArray = *pEmxArray;
  emxArray->data = (c_struct_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxInit_struct_T_400(emxArray_struct_T_400 *pEmxArray)
{
  pEmxArray->size[0] = 0;
}

void emxInit_uint32_T(emxArray_uint32_T **pEmxArray, int32_T numDimensions,
                      boolean_T doPush)
{
  emxArray_uint32_T *emxArray;
  int32_T i;
  *pEmxArray = (emxArray_uint32_T *)emlrtMallocMex(sizeof(emxArray_uint32_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(emlrtRootTLSGlobal, (void *)pEmxArray,
      (void *)&emxFree_uint32_T);
  }

  emxArray = *pEmxArray;
  emxArray->data = (uint32_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxInit_uint8_T(emlrtCTX aTLS, emxArray_uint8_T **pEmxArray, int32_T
                     numDimensions, boolean_T doPush)
{
  emxArray_uint8_T *emxArray;
  int32_T i;
  *pEmxArray = (emxArray_uint8_T *)emlrtMallocMex(sizeof(emxArray_uint8_T));
  if (doPush) {
    emlrtPushHeapReferenceStackR2012b(aTLS, (void *)pEmxArray, (void *)
      &emxFree_uint8_T);
  }

  emxArray = *pEmxArray;
  emxArray->data = (uint8_T *)NULL;
  emxArray->numDimensions = numDimensions;
  emxArray->size = (int32_T *)emlrtMallocMex(sizeof(int32_T) * numDimensions);
  emxArray->allocatedSize = 0;
  emxArray->canFreeData = true;
  for (i = 0; i < numDimensions; i++) {
    emxArray->size[i] = 0;
  }
}

void emxTrim_struct_T(b_emxArray_struct_T *emxArray, int32_T fromIndex, int32_T
                      toIndex)
{
  int32_T i;
  for (i = fromIndex; i < toIndex; i++) {
    emxFreeStruct_struct_T2(&emxArray->data[i]);
  }
}

void emxTrim_struct_T_400(b_struct_T data[400], int32_T fromIndex, int32_T
  toIndex)
{
  int32_T i;
  for (i = fromIndex; i < toIndex; i++) {
    emxFreeStruct_struct_T(&data[i]);
  }
}

/* End of code generation (SkinDetect_emxutil.c) */
