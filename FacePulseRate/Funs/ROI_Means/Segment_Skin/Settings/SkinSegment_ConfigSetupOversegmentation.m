function SkinSegmentConfig = ...
             SkinSegment_ConfigSetupOversegmentation(OversegmentationCheckTF, ...
                 SkinSegmentConfig, VideoReadConfig)             
%SkinSegment_ConfigSetupOversegmentation   Settings for the over-segmentation check and adjustment.
%
...
%    -----------
%
%    Settings for the over-segmentation check and adjustment, which is conducted by functions
%    SkinSegment_OversegmentationCheck and SkinSegment_OversegmentationCheck_Reduce. Function
%    SkinSegment_OversegmentationCheck is called by function ROIMeans_FirstRead after each 
%    face-detection attempt that meets certain criteria. Function SkinSegment_OversegmentationCheck  
%    checks for oversegmentation. If present, it signals to function 
%    SkinSegment_OversegmentationCheck_Reduce to reduce the settings of various skin-segmentation
%    settings to attempt to reduce oversegmentation. 
%
%
%    Description of Function SkinSegment_OversegmentationCheck
%    ---------------------------------------------------------
%
...
%    -- Note --
%
%    A copy of this description is located in function SkinSegment_OversegmentationCheck.
%
%
%    License
%    -------
...
%%%%%% Whether to enable the oversegmentation check and adjustment %%%%%%

%OversegmentationCheckTF is an argument to function FacePulseRate.

SkinSegmentConfig.OverSegmentedCheck.CheckTF = OversegmentationCheckTF;


%%%%%% Oversegmentation detection settings %%%%%%%

%Specify the skin-proportion thresholds below which the ROIs will be considered to be 
%oversegmented:

%Different thresholds are used because the face-detection algorithms are expected to have different 
%typical proportions. The proportion should be smaller for a given face-detection algorithm if less  
%skin area is expected to be present, on average, in the ROI it returns. The ROI of a detection 
%algorithm may have less skin area if ... 
...
%Note that the skin-segmentation settings in effect for the proportions used as input to 
%SkinSegment_OversegmentationCheck are different from the skin-segmentation settings in effect when
%writing frames to the output video. Specifically, the skin-segmentation settings in effect for
%SkinSegment_OversegmentationCheck use intentionally severe settings to filter out false positive
%face detections, whereas the skin-segmentation settings in effect for the frames written to the
%video used low-severity settings. Consequently, if using the output video to calibrate the
%proportions below, note that lower proportions will be present than those that appear in the 
%video.

SkinSegmentConfig.OverSegmentedCheck.ProportionThresholdPrimary    = single(.13); 
SkinSegmentConfig.OverSegmentedCheck.ProportionThresholdSecondary1 = single(.10);
SkinSegmentConfig.OverSegmentedCheck.ProportionThresholdSecondary2 = single(.12);

%Specify threshold for the proportion of frames out of a specified number of frames back where the 
%proportion of skin was below the skin-proportion threshold. The number of frames back is specified
%by SkinSegmentConfig.OverSegmentedCheck.NFramesLowSkinProportionStatus.
%length of SkinSegmentConfig.OverSegmentedCheck.FramesLowSkinProportionStatus (see below). When  
%this threshold is exceeded, oversegmentation is considered present. In response, the severity of
%skin-segmentation settings is reduced. The skin-segmentation settings are found in   
%SkinSegmentConfig.Args.
SkinSegmentConfig.OverSegmentedCheck.FramesLowSkinProportionThreshold = double(.90);

%Specify how many frames back ROIs will be checked for a low proportion of pixels classified as
%skin. This determines how many frames will be included when calculating the frames-low-skin 
%proportion, which indicates the proportion of frames that were below the skin-proportion 
%threshold. The value is adjusted by frame rate.
%Specify as a floating-point type because this is used as the denominator to return a 
%floating-point value.

Duration = VideoReadConfig.EndTime - VideoReadConfig.StartTime;

%If the duration of the video is greater than 2 minutes
%A different number of frames is used for shorter input videos.
if Duration > 120
    
    SkinSegmentConfig.OverSegmentedCheck.NFramesLowSkinProportionStatus = ...
        double( ...
            round( ...
                16.66 * VideoReadConfig.FS ...
            ) ...
        );
    
else

    %Use a sliding value based upon the duration. Also specify a minimum.
    %When setting this value, consider that many frames may be intentionally skipped by the
    %face-detection algorithms.
    
    SkinSegmentConfig.OverSegmentedCheck.NFramesLowSkinProportionStatus = ...
        double( ...
            max( ...  
                round( Duration * VideoReadConfig.FS / 60 ), ...
                10 ... minimum
            ) ...
        );        
end  
...
%%%%%%% Less-severe morphological close settings %%%%%%

%Specify the amount by which to reduce SkinSegmentSeverityFactor
%SkinSegmentSeverityFactor controls the severity of the morphological close operations conducted
%by skin segmentation. SkinSegmentSeverityFactor is an argument to function FacePulseRate. 
SkinSegmentConfig.OverSegmentedCheck.MorphReductionAmount = .2;

%Specify the maximum number of times the severity of SkinSegmentSeverityFactor can be reduced.
SkinSegmentConfig.OverSegmentedCheck.NMorphReductionLimit = int32(2);

%Initialize count of the number of times SkinSegmentSeverityFactor has been decreased
%Reassigned by function SkinSegment_OversegmentationCheck.
%This value is also displayed on the legend of the output video.
SkinSegmentConfig.OverSegmentedCheck.NMorphReductions = int32(0);


%%%%%%% Dark ROIs detection settings %%%%%%

%Specify how many frames back ROIs will be checked for darkness. This determines how many frames 
%will be included when calculating the mean brightness level across frames. This brightness level
%will be checked against threshold SkinSegmentConfig.OverSegmentedCheck.DarknessThreshold. The 
%value is adjusted by frame rate.
%Specify as a floating-point type because this is used as the denominator to return a 
%floating-point value.
...
if Duration > 120
    ...
    SkinSegmentConfig.OverSegmentedCheck.NFramesYSingleMean = ...
         single( ...
            round( ...
                2.08 * VideoReadConfig.FS ...
            ) ...
        );
    ...
    SkinSegmentConfig.OverSegmentedCheck.NFramesYSingleMean = ...
        single( ...
            max( ... 
                round(  Duration * VideoReadConfig.FS / 240 ), ...
                3 ... minimum
            ) ...            
        );        
end    
    
%Specify the darkness threshold (Y-colorspace mean) under which the ROIs are considered to be dark
%(low brightness).
SkinSegmentConfig.OverSegmentedCheck.DarknessThreshold = single(50);

%Initialize flag whether Y-colorspace mean is below darkness threshold 
SkinSegmentConfig.OverSegmentedCheck.BelowDarknessThresholdTF = false;


%%%%%%% Less-severe pixel-color threshold settings %%%%%%

%Initialize flag whether the pixel-color thresholds were adjusted to be less severe
SkinSegmentConfig.OverSegmentedCheck.PixelColorsAdjustedTF = false;

%Specify the pixel-color thresholds to assign if 
%SkinSegmentConfig.OverSegmentedCheck.BelowDarknessThresholdTF is true. NaN indicates that the
%threshold will not be reassigned. All elements can accept a number value and all elements can 
%accept an NaN value.
SkinSegmentConfig.OverSegmentedCheck.ColorThresAdjustTo = ...
    single( ...
        [21,   ... Y minimum
         NaN,  ... Cb minimum
         134,  ... Cb maximum
         128,  ... Cr minimum
         NaN,  ... Cr maximum
         NaN,  ... Cb-to-Cr minimum 
         1.06, ... Cb-to-Cr maximum 
         .99,  ... H maximum
         .03]  ... S minimum
    );
