 ...
 *
 * insertTextFast_ROIAnnotation13.c
 *
 * Code generation for function 'insertTextFast_ROIAnnotation13'
 *
 */
...
void insertTextFast_ROIAnnotation13(uint8_T VidFrameCrop[47850], const char_T
  Text_data[], const int32_T Text_size[2])
{
  ...
  /*                                  Intended only for C-language code generation. */
  /*  */
        ...
        for (c = tbTopLeftX; c <= endC; c++) {
          for (r = startR - 13; r <= endR; r++) {
            ...
            tmp22 = (uint8_T)(0.4 * (real_T)VidFrameCrop[((r + 29 * (c - 1)) +
              15950 * tbWidth) - 1] + 0.5);
            VidFrameCrop[((r + 29 * (c - 1)) + 15950 * tbWidth) - 1] = (uint8_T)
...
/* End of code generation (insertTextFast_ROIAnnotation13.c) */
